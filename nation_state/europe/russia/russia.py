...
def main(time1):
    russian = Russia(time1)
    if not russian.sprite:
        globe1 = globe.Globe()

        #upload_database.initial_upload_to_database(globe1.nations)
        while russian.population > 6000000:
            print(f"Current Date: {russian.date}\n")
            time.sleep(1.5)
            russian.population_change()
            russian.check_economic_state()
            russian.stability_happiness_change(globe1)
