        ...
        if globe.tension > 25 and globe.tension < 50:
            ...
            if self.improve_happiness > self.date:
                ...
                if (self.happiness + happiness_increase) < 100:
                    self.happiness += happiness_increase
def main(time1):
    dutch = Netherlands(time1)
    if not dutch.sprite:
        globe1 = globe.Globe()

        #upload_database.initial_upload_to_database(globe1.nations)
        while dutch.population > 6000000:
            print(f"Current Date: {dutch.date}\n")
            time.sleep(1.5)
            dutch.population_change()
            dutch.check_economic_state()
            dutch.stability_happiness_change(globe1)
