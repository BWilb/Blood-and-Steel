...
def manual_game(year):
    serbia = Serbia(year)
    while serbia.current_pop > 3000000:
        political_change(serbia)
        economic_decisions(serbia)
        population_change(serbia)
        if serbia.date > serbia.check_stats:
            daily_decisions(serbia)
        serbia.date += timedelta(days=1)
        time.sleep(3)
