        ...
        if globe.tension > 25 and globe.tension < 50:
            ...
            if self.improve_happiness > self.date:
                ...
                if (self.happiness + happiness_increase) < 100:
                    self.happiness += happiness_increase
def main(time1):
    luxembourger = Luxembourg(time1)
    if not luxembourger.sprite:
        globe1 = globe.Globe()

        #upload_database.initial_upload_to_database(globe1.nations)
        while luxembourger.population > 6000000:
            print(f"Current Date: {luxembourger.date}\n")
            time.sleep(1.5)
            luxembourger.population_change()
            luxembourger.check_economic_state()
            luxembourger.stability_happiness_change(globe1)
